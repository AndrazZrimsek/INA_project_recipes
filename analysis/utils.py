...
def find_node(G: nx.Graph, label: str, label_key="label"):
    """Finds node with given label in G."""
    for i, data in G.nodes(data=True):
        if data[label_key] == label:
            return i
    raise ValueError(f"node '{label}' not found in {G.name}")


def top_nodes(
    G: nx.Graph, C: Dict[Any, float], centrality: str, n=15, typ=None
) -> List[Any]:
    ...
    for i, c in sorted(
        ...
        n = G.nodes(data=True)[i]
        if typ != None and n["data"]["type"] != typ:
            continue
        nodes.append(n)
        print(i)
        # print("{:>12.6f} | '{:s}' ({:,d})".format(c, n["title"], G.degree[i]))
        n -= 1
        if n == 0:
            break
    print()
...
def pagerank(G: nx.Graph, alpha=0.85, eps=1e-6, teleport: Set | None = None):
    ...
    diff = 1
    while diff > eps:
        U = [sum([P[j] * alpha / G.degree(j) for j in G[i]]) for i in G]
